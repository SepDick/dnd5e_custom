...
export default class ItemListControlsElement extends HTMLElement {
   ...
   */
  get prefs() {
    return game.user.getFlag("dnd5e_custom", `sheetPrefs.${this.app.object.type}.tabs.${this.tab}`);
  }
  ...
  async _onToggleMode(event) {
    ...
    const current = game.user.getFlag("dnd5e_custom", flag);
    let value;
    ...
    await game.user.setFlag("dnd5e_custom", flag, value);
    if ( action === "group" ) {
      this._initGrouping();
      this._applyGrouping();
    } else if ( action === "sort" ) {
