...
export default class JournalClassPageSheet extends JournalPageSheet {
  ...
  /** @inheritdoc */
  get template() {
    return `systems/dnd5e_custom/templates/journal/page-${this.document.type}-${this.isEditable ? "edit" : "view"}.hbs`;
  }
  ...
  async _getSpellProgression(item) {
    ...
    if ( spellcasting.type === "leveled" ) {
       ...
       * The actual hook names include the spellcasting type (e.g. `dnd5e_custom.buildPsionicSpellcastingTable`).
       ...
       * @function dnd5e_custom.buildSpellcastingTable
       * @memberof hookEvents
       */
      Hooks.callAll(
        `dnd5e_custom.build${spellcasting.type.capitalize()}SpellcastingTable`, table, item, spellcasting
      );
    }
