...
/**
 * A data model and API layer which handles the schema and functionality of "group" type Actors in the dnd5e_custom system.
 ...
 * const g = new dnd5e_custom.documents.Actor5e({
 *  type: "group",
 *  name: "Test Group",
 *  system: {
...
export default class GroupActor extends ActorDataModel.mixin(CurrencyTemplate) {
  ...
  async rest(config, result) {
     ...
     * @function dnd5e_custom.groupRestCompleted
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("dnd5e_custom.groupRestCompleted", this.parent, results);
  ...
  _onUpdate(changed, options, userId) {
    if ( !foundry.utils.hasProperty(changed, "system.type.value") || (game.user !== game.users.activeGM)
      || (game.settings.get("dnd5e_custom", "primaryParty")?.actor !== this.parent)
